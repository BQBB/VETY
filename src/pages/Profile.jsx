    ...
    <Container>
        ...
        <GridItem style='md:col-span-4 mb-10 flex flex-col items-center md:items-start'>
            ...
            <ul className="mt-4 sm:mt-10 flex flex-col gap-y-2">
                <p className="text-lg sm:text-vmd text-center md:text-right">{(user.first_name+" "+user.last_name).trim()}</p>
                {user.phone_number ? 
                ...
                {user.address ? 
                    <li className="flex items-center gap-x-4">
                        ...
                        <p className="text-vsm sm:text-lg">{user.address.city.name +" - "+ user.address.zone.name}</p>
                    </li>
                :
                null
                }
                            ...
                            return (
                                <Link to='/pet'>
                                    <div key={i} className="flex flex-col items-center">
                                        <img className="rounded-full border border-vblue h-20 w-20 md:h-28 md:w-28" src={BASE_URL+pet.image.slice(1,pet.image.length)} alt='pet img' />
                                        <p className="text-vsm sm:text-lg mt-1">{pet.name}</p>
                                    </div>
                                </Link>
                            )
                        })
