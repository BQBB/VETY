...
const Contact = () => {
    ...
    const handleSubmit = (e) => {
        ...
        (new Http).post("/api/contact/contact_us",{'first_name':fname, 'last_name':lname, 'email':email, 'question':question}).then(
            res => {
        ...
        ).catch(err => setMsg({text:'حدثت مشكلة اثناء ارسال الرسالة' ,category:'vred'}))
    }
            ...
            <div className='mt-14'>
                <Grid style='items-center gap-x-4'>
                    ...
                    <form className='col-span-12 sm:col-span-6 bg-vgray p-5 rounded-md' onSubmit={handleSubmit} >
                        <Input for='fname' type='text' label='الاسم الأول' value={fname} onType={(e) => setFname(e.target.value)} />
                        { errors.fname && <AlertMsg msg={errors.fname} category='vred' /> }
                        <Input for='lname' type='text' label='الاسم الأخير' value={lname} onType={(e) => setLname(e.target.value)} />
                        { errors.lname && <AlertMsg msg={errors.lname} category='vred' /> }
                        <Input for='email' type='email' label='البريد الألكتروني' value={email} onType={(e) => setEmail(e.target.value)} />
                        { errors.email && <AlertMsg msg={errors.email} category='vred' /> }
                        <div className='flex flex-col mt-4'>
                            <label className='text-[#102840] text-vsm'>ادخل سؤالك هنا</label>
                            <textarea value={question} onChange={(e) => setQuestion(e.target.value)} name='question' className='p-2 border-[2px] border-vblue focus:ouline-none rounded-md'></textarea>
                            { errors.question && <AlertMsg msg={errors.question} category='vred' /> }
                        </div>
